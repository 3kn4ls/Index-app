...
# 2. Importar imagen a k3s
print_info "Paso 2/5: Importando imagen a k3s..."
docker save ${IMAGE_NAME}:v${NEW_VERSION} -o /tmp/${IMAGE_NAME}-v${NEW_VERSION}.tar
sudo k3s ctr images import /tmp/${IMAGE_NAME}-v${NEW_VERSION}.tar
rm /tmp/${IMAGE_NAME}-v${NEW_VERSION}.tar

# Importar también el tag latest
docker save ${IMAGE_NAME}:latest -o /tmp/${IMAGE_NAME}-latest.tar
sudo k3s ctr images import /tmp/${IMAGE_NAME}-latest.tar
rm /tmp/${IMAGE_NAME}-latest.tar
